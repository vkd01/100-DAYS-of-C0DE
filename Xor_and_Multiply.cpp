...
void genius(ll a, ll b, ll i, ll fuck, ll res){
    if( ((1<<i)&a)==0 && ((1<<i)&b)>0) {
                fuck = 0;
            }
            else {
...
ll fuck = -1;
for(int i=n-1;i>=0;i--){
    if( ((BIN(1,i))&a)>0 && ((BIN(1,i))&b)>0 ) continue;

    else if(((BIN(1,i))&a)==0 && ((BIN(1,i))&b)==0) res |=(1<<i);

    else {
        if(fuck==-1) {
            genius( a,  b,  i,  fuck,  res);
        }
        else {
            if(((1<<i)&a)==0 && ((BIN(1,i))&b)>0){
                res|=(BIN(1,i));
            }
        }
    } 
