...
vector<int> bfsOfGraph(int V, vector<int> adj[]) {
   ...
   ll max_len = 0;
ll min_index(vl &v, ll start, ll end) {
   ll ind = 0, temp = INT_MAX;
...
void selection_sort(vl &v, int start, int end) {
   if (start >= end) return;

   int min_ind = min_index(v, start, end);
   swap(v[start], v[min_ind]);
